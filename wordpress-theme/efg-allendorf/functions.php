...
function efga_gruppe_meta_cb( $post ) {
	...
	efga_field( $post->ID, 'efga_emoji', 'Emoji / Symbol', '📖' );
	efga_field( $post->ID, 'efga_badge', 'Zielgruppe (Badge)', 'Alle Erwachsene' );
	efga_field( $post->ID, 'efga_gradient_from', 'Farbverlauf von', '#1e4b8a', 'Hex-Farbe, z. B. #1e4b8a' );
	efga_field( $post->ID, 'efga_gradient_to', 'Farbverlauf bis', '#2d6cc0', 'Hex-Farbe, z. B. #2d6cc0' );
...
function efga_veranstaltung_meta_cb( $post ) {
	...
	efga_field( $post->ID, 'efga_accent', 'Akzentfarbe Kopf', '#1e4b8a', 'Optional, Standard: #1e4b8a' );
...
function efga_save_meta( $post_id ) {
	...
	$gruppe_keys = array( 'efga_emoji', 'efga_badge', 'efga_gradient_from', 'efga_gradient_to', 'efga_schedule', 'efga_location', 'efga_audience', 'efga_hero_subtitle', 'efga_short', 'efga_leader_name', 'efga_leader_initials', 'efga_leader_phone', 'efga_leader_email' );
	...
	$va_keys = array( 'efga_date_day', 'efga_date_month', 'efga_time', 'efga_tag', 'efga_accent', 'efga_link' );
...
function efga_default_menu() {
	echo '<nav>';
	echo '<a href="' . esc_url( home_url( '/' ) ) . '"' . ( is_front_page() ? ' class="active"' : '' ) . '>Home</a>';
	...
	echo '<a href="' . esc_url( get_permalink( get_page_by_path( 'gottesdienst-live' ) ) ) . '" style="color:#e53935;font-weight:700;">🔴 Live</a>';
	echo '<a href="' . esc_url( get_permalink( get_page_by_path( 'kalender' ) ) ) . '">Kalender</a>';
	echo '<a href="' . esc_url( get_permalink( get_page_by_path( 'intern' ) ) ) . '" class="nav-intern">🔒 Intern</a>';
	echo '</nav>';
}
